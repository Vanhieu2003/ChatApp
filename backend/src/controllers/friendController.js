...
export const sendFriendRequest = async (req,res) => {
    try {
        ...
        if (from.toString() === to.toString()) {
            return res.status(400).json({message: "Không thể tự kết bạn chính mình."})
        }
...
export const acceptFriendRequest = async (req,res) => {
    try {
        ...
        return res.status(200).json({message:"Chấp nhận lời mời thành công.", newFriend: {
            _id: from?._id,
...
export const declineFriendRequest = async (req,res) => {
    try {
        ...
        if(request.to.toString() !== request.from.toString()){
